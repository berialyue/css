...
clear 属性只有块级元素才有效果，而 ::after 等伪元素默认都是内联元素，这就是借助伪元素清除浮动影响时需要设置 display 属性的原因
...
#### 8.3.2 具有近似姐妹花属性值得 font-style

font-style 标识文字造型是否为斜体

属性值 italic 是使用当前字体的斜体字体，oblique 是单纯地让文字倾斜

#### 8.3.3 不适合国庆的 font-variant

font-variant 可以让英文字符表现为小体型大写字母，就是大小跟小写字母一样，但样式是大写字母
...
font-size 和 font-family 是必须的属性，如果 font 属性无效，检查一下这两个属性是否同时存在

在使用 font 缩写的时候，如果不设定行高值，一定会出现不兼容问题

font 缩写一定要带上 font-family，解决方法：
...
对于 IE8，字体图标技术依然是非常值得推荐的技术选型

字体图标技术就是把通常的字符映射成为另外的图标形状

字体图标技术的一些渲染现象：

1. 因为原始字符和最终的图形变现相差很大，所以当我们的字符文件加载缓慢的时候，可以明显看到字符变图形的过程。字体内联在 CSS 文件中可以有效避免这一问题，但往往字体文件都比较大，这样处理得不偿失
2. 原始的字符 x-height 和最终的图形 x-height 往往不一样，这样会印象内联元素的垂直对齐，很容易出现页面高度闪动的情况
3. 原始字符的 ch 宽度，也就是水平占据的宽度和最终的图形也是不一样的，因此很容易出现内联元素水平方向晃动的问题
